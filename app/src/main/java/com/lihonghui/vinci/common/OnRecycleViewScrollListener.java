package com.lihonghui.vinci.common;
...
public abstract class OnRecycleViewScrollListener extends RecyclerView.OnScrollListener {
	...
	public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
		...
		if ((visibleItemCount > 0 && currentScrollState == RecyclerView.SCROLL_STATE_IDLE && (lastVisibleItemPosition) >= totalItemCount - 1)) {
			View lastView = layoutManager.getChildAt(visibleItemCount - 1);
			//LogUtil.printLog("onScrollStateChanged  lastView.getBottom():"+lastView.getBottom()+"  recyclerView.getBottom():"+recyclerView.getBottom());

			if(lastView.getBottom() == recyclerView.getBottom()){
				onScrollBottom();
			}
		} else if ((visibleItemCount > 0 && currentScrollState == RecyclerView.SCROLL_STATE_IDLE && (firstVisibleItemPosition) == 0)) {
			onScrollTop();
		}

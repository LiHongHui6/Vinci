...
public class CardSlidePanel extends FrameLayout {
    private List<BaseCardItemView> viewList = new ArrayList<BaseCardItemView>(); // 存放的是每一层的view，从顶到底
    ...
    //视图布局完成后会调用computeScroll，为避免错误回调
            ...
            public void onClick(View view) {
               KLog.e("onClick ~~~~~~~~~~~~~~~~");
                if (null != cardSwitchListener && view.getScaleX() > 1 - SCALE_STEP) {
                    KLog.e("onClick 222222222222222");
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        public void onViewPositionChanged(View changedView, int left, int top,
                                          int dx, int dy) {
            onViewPosChanged((BaseCardItemView) changedView);
        }
        ...
        public boolean tryCaptureView(View child, int pointerId) {
            ...
            if (childIndex > 0) {
//                KLog.e("3 childIndex:"+childIndex + " child tagC:"+child.getTag()+"  child tagR:"+viewList.get(0).getTag());
//                testGetViewIndex();

                return false;
            }

            ((BaseCardItemView) child).onStartDragging();
            boolean shouldCapture = ((BaseCardItemView) child).shouldCapture(downPoint.x, downPoint.y);
            if (shouldCapture) {
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            animToSide((BaseCardItemView) releasedChild, (int) xvel, (int) yvel);
        }
...
//        KLog.e("onViewPosChanged");
    ...
    private void orderViewStack() {
        //  KLog.e("orderViewStack");
        if (releasedViewList.size() == 0) {
            return;
        }

        BaseCardItemView changedView = (BaseCardItemView) releasedViewList.get(0);
        ...
        for (int i = num - 1; i > 0; i--) {
            BaseCardItemView tempView = viewList.get(i);
            tempView.setAlpha(1);
    ...
    private void processLinkageView(View changedView) {
        ...
        BaseCardItemView bottomCardView = viewList.get(viewList.size() - 1);
     ...
     * 松手时处理滑动到边缘的动画
     */
    private void animToSide(BaseCardItemView changedView, int xvel, int yvel) {
    ...
    public void computeScroll() {
        if (mDragHelper.continueSettling(true)) {
            ...
            // 动画结束
            if (mDragHelper.getViewDragState() == ViewDragHelper.STATE_IDLE && !isRefresh) {
                // KLog.e("computeScroll child size: " + getChildCount());
                orderViewStack();
                btnLock = false;
            } else {
                isRefresh = false;
    ...
    private void initViewList() {
        ...
        for (int i = num - 1; i >= 0; i--) {
            ...
            childView.setTag("index: " + i);
            // for循环取view的时候，是从外层往里取
            BaseCardItemView viewItem = (BaseCardItemView) childView;
            viewItem.setParentView(this);
            viewItem.setTag(i + 1);
            viewItem.rootView.setOnClickListener(btnListener);
    ...
    protected void onFinishInflate() {
        ...
        // 渲染完成，初始化卡片view列表
        //  KLog.e("onFinishInflate()   isShowing: "+isShowing);
        isShowing = 0;
        initViewList();
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right,
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i > viewListSize - 2) {
                // 备用的view
                offset = yOffsetStep * (viewListSize - 2);
                scale = 1 - SCALE_STEP * (viewListSize - 2);
            }
    ...
    public void fillData(List dataList) {
        ...
        int num = viewList.size();
        for (int i = 0; i < num; i++) {
            BaseCardItemView itemView = viewList.get(i);
...
//    private void testGetViewIndex(){
//        for (int index = 0;index<getChildCount();index++){
//           // KLog.e(getChildAt(index).getTag());
//        }
//    }
